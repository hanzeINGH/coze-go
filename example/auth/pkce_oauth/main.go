...
func main() {
	...
	oauth, err := coze.NewPKCEOAuthClient(clientID, coze.WithBaseURL(cozeAPIBase))
	if err != nil {
	...
	*/
	oauthURL, err := oauth.GenOAuthURL(redirectURI, "state", nil)
	if err != nil {
	 ...
	 * oauthURL, err = oauth.GenOAuthURLWithWorkspace(redirectURI, "state", "workspaceID", coze.CodeChallengeMethodS256.Ptr());
	 * if err != nil {
